      ...
      login: async (email: string, password: string) => {
        try {
          ...
          if (data.user) {
            ...
            // ✅ CRITICAL FIX: Identify user with Stripe BEFORE setting auth state
            // This prevents race condition where subscription check happens on anonymous user
            try {
              await identifyStripeUser(data.user.id);
              console.log('✅ Stripe user identified before setting auth state');
            } catch (error) {
              console.warn('⚠️ Failed to identify Stripe user during login:', error);
            }
        ...
        try {
          ...
          if (data.user) {
            ...
            try {
              ...
              console.warn('⚠️ Failed to identify Stripe user during signup:', error);
            }
